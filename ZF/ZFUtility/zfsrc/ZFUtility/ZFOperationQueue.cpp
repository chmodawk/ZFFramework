...
ZFOBJECT_SINGLETON_DEFINE_DETAIL(ZFOperationQueue, ZFOperationQueue, ZFOperationQueue, instance, ZFLevelZFFrameworkHigh)
...
static void _ZFP_ZFOperationQueue_checkStopQueue(ZF_IN ZFOperationTaskData *queueTaskData, ZF_IN ZFOperationQueueResult *queueResult, ZF_IN _ZFP_ZFOperationQueueTaskState *taskState)
{
    if(taskState->childRunning->isEmpty() && taskState->childToRun->isEmpty())
    {
        ...
        queueTaskData->operationOwner()->taskNotifyFinish(queueTaskData->operationId(), queueResult);
    }
}
                                                      ...
                                                      ZF_IN zfindex childIndex)
{
    ...
    queueTaskData->operationOwner()->taskNotifyProgress(queueTaskData->operationId(), queueProgress);
}
