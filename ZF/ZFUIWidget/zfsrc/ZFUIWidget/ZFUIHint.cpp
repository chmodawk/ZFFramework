...
public:
    ...
    void hintDoFinish(void)
    {
        ...
        ZFPropertyChangeWithoutLeakTest(this->hintAnimating, zfnull);
        ZFArrayEditable *hintList = _ZFP_ZFUIHint_hintListForWrite(this->pimplOwner->hintWindow()->windowSysWindow());
        ...
        ZFLeakTestLogBeforeReleaseVerbose(this->pimplOwner, ZFCallerInfo(ZF_CALLER_FILE, zfTextA("hintHide"), ZF_CALLER_LINE));
...
void ZFUIHint::hintShow(void)
{
    ...
    ZFArrayEditable *hintList = _ZFP_ZFUIHint_hintListForWrite(this->hintWindow()->windowSysWindow());
    hintList->add(this);
    if(hintList->count() == 1)
    {
        d->hintDoShow();
...
ZFObject *ZFUIHint::objectOnInit(void)
{
    ...
    ZFLISTENER_LOCAL(hintWindowChanged, {
        ...
        ZFArrayEditable *hintListNew = _ZFP_ZFUIHint_hintListForWrite(hint->hintWindow()->windowSysWindow());
