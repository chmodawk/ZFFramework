...
public:
    ...
    virtual zfbool childRemoveForDealloc(ZF_IN ZFUIView *parent)
    {
        ...
        if(nativeView.subviews.count > 0
            && !(nativeView._ZFP_nativeImplView != nil && nativeView.subviews.count == 1))
        {
            while(nativeView.subviews.count > 0)
            {
                [nativeView.subviews.lastObject removeFromSuperview];
            }
            [nativeView addSubview:nativeView._ZFP_nativeImplView];
        }
        return zftrue;
    }
