...
ZF_GLOBAL_INITIALIZER_INIT_WITH_LEVEL(ZFEncryptUpgradeDataHolder, ZFLevelZFFrameworkStatic)
{
}
public:
...
ZFMETHOD_DEFINE_2(ZFEncrypt, zfbool, encrypt,
...
ZFMETHOD_DEFINE_2(ZFEncrypt, zfbool, decrypt,
                  ZFMP_IN_OUT(const ZFOutputCallback &, output),
                  ZFMP_IN(const ZFInputCallback &, input))
{
    if(!input.callbackIsValid() || !output.callbackIsValid())
                                 ...
                                 ZF_IN_OPT const zfchar *encryptKey /* = zfnull */)
{
    ...
    if(!datas.isEmpty())
    {
        ...
        this->encryptProtocolT(upgradeTaskData.encryptProtocol);
        this->encryptProtocolVersionT(upgradeTaskData.encryptProtocolVersion);
